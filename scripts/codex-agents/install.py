#!/usr/bin/env python3
"""Install, check, update, or uninstall David's generated Codex adapter."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if manifest.get("manager") != "bsv-skills" or [a.get("generated_file") for a in manifest.get("agents", [])] != [AGENT_FILE]:
        print("error: invalid or missing BSV Skills agent manifest")
        return 1
    ...
    ownership = load_json(ownership_path, {"manager": "bsv-skills", "agents": {}})
    if ownership.get("manager") != "bsv-skills":
        print(f"error: invalid BSV Skills ownership metadata: {ownership_path}")
        return 1
    record = ownership.get("agents", {}).get(AGENT_FILE)

    if args.uninstall:
        if not record:
            print(f"unchanged: {AGENT_FILE} is not managed by BSV Skills")
            return 0
    ...
    print("Start a new Codex session before invoking bsv_skills_bitcoin.")
    return 0
